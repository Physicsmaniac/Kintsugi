...
# --- PAGE CONFIG ---
st.set_page_config(page_title="Model Comparison Testing", layout="wide")
...
def score_pairs(model, images_pil, device):
    """Score all pairs of images with a model"""
    n = len(images_pil)
...
# --- UI LAYOUT ---
st.title("Forensic Document Reconstructor - Model Comparison")
...
with nav_col1:
    if st.button("🧩 Solver (Compare)", use_container_width=True, type="primary" if st.session_state.active_tab == "🧩 Solver" else "secondary"):
        st.session_state.active_tab = "🧩 Solver"
        st.rerun()
...
if active_tab == "📄 Shredder":
    ...
    st.header("Reconstruction Solver - Compare 2 Models")
    
    # Get list of available .pth files
    pth_files = list(Path("/home/ryanl/Documents/Compvis V2").glob("*.pth"))
    pth_files = sorted([f.name for f in pth_files])
    ...
    with st.expander("Solver Settings", expanded=True):
        ...
        with col2:
            ...
            model2_name = st.selectbox(
                "Choose second model",
                pth_files,
                index=1 if len(pth_files) > 1 else 0,
                key="model2_select"
            )
    ...
    if input_files:
        ...
        # 2. RUN BUTTON
        if st.button("Compare Models on This Dataset", type="primary", use_container_width=True):
            if model1_name == model2_name:
                st.error("Please select two different models to compare")
            else:
                model1_path = f"/home/ryanl/Documents/Compvis V2/{model1_name}"
                model2_path = f"/home/ryanl/Documents/Compvis V2/{model2_name}"
                
                with st.spinner("Loading models..."):
                    model1, device1 = load_model(model1_path)
                    model2, device2 = load_model(model2_path)

                if not model1 or not model2:
                    st.error("Failed to load one or both models")
                else:
                    st.success(f"✓ Loaded {model1_name} and {model2_name}")
                    
                    with st.spinner("Processing images..."):
                        images_pil, images_cv = process_uploads(input_files)
                        n = len(images_pil)
                    
                    st.success(f"✓ Processed {n} images")
                    
                    # Score with both models
                    st.header("Scoring Images...")
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        st.subheader(f"Scoring with {model1_name}...")
                        score_matrix_m1 = score_pairs(model1, images_pil, device1)
                        st.success(f"✓ {model1_name} complete")
                    
                    with col2:
                        st.subheader(f"Scoring with {model2_name}...")
                        score_matrix_m2 = score_pairs(model2, images_pil, device2)
                        st.success(f"✓ {model2_name} complete")
                    
                    # --- SOLVE WITH BOTH MODELS ---
                    st.header("Reconstruction Solver - Model Comparison")
                    
                    found_pages_m1 = solve_mixed_bag(score_matrix_m1, n, confidence_thresh)
                    found_pages_m2 = solve_mixed_bag(score_matrix_m2, n, confidence_thresh)
                    ...
                    with col_left:
                        ...
                        fig1 = px.imshow(score_matrix_m1, color_continuous_scale='RdBu_r', origin='upper')
                        fig1.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=350)
                        st.plotly_chart(fig1, use_container_width=True)
                        
                        mask = np.eye(n, dtype=bool)
                        scores_m1_off_diag = score_matrix_m1[~mask]
                        st.metric("Mean Score", f"{np.mean(scores_m1_off_diag):.4f}")
                        st.metric("Pages", len(found_pages_m1))
                        
                        st.markdown("---")
                        st.subheader(model2_name)
                        fig2 = px.imshow(score_matrix_m2, color_continuous_scale='RdBu_r', origin='upper')
                        fig2.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=350)
                        st.plotly_chart(fig2, use_container_width=True)
                        
                        scores_m2_off_diag = score_matrix_m2[~mask]
                        st.metric("Mean Score", f"{np.mean(scores_m2_off_diag):.4f}")
                        st.metric("Pages", len(found_pages_m2))
                    
                    with col_right:
                        st.markdown("### Results")
                        st.caption("Hover and click the full-screen arrows to view details.")
                        
                        if not found_pages_m1 and not found_pages_m2:
                            st.warning("No pages found with either model. Try lowering threshold.")
                        else:
                            # Show results from both models
                            max_pages = max(len(found_pages_m1), len(found_pages_m2))
                            ...
                            for idx in range(max_pages):
                                ...
                                with col_m1:
                                    st.caption(f"**{model1_name}**")
                                    if idx < len(found_pages_m1):
                                        chain = found_pages_m1[idx]
                                        # Stitch
                                        ...
                                        st.image(
                                            full_page_bgr,
                                            caption=f"{len(chain)} strips",
                                            channels="BGR",
                                            use_container_width=True
                                        )
                                    else:
                                        st.info("No page found")
                                
                                with col_m2:
                                    st.caption(f"**{model2_name}**")
                                    if idx < len(found_pages_m2):
                                        chain = found_pages_m2[idx]
                                        # Stitch
                                        page_imgs = [images_cv[c] for c in chain]
                                        target_h = page_imgs[0].shape[0]
                                        resized_imgs = []
